...
namespace btcm { namespace example_plugin {

class hello_api_plugin : public btcm::app::plugin
{
   public:
      /**
...
{
   public:
      hello_api_api( const btcm::app::api_context& ctx );
   ...
   private:
      btcm::app::application& _app;
      uint32_t _api_call_count = 0;
};

} }

FC_API( btcm::example_plugin::hello_api_api,
   (get_message)
   )
...
hello_api_api::hello_api_api( const btcm::app::api_context& ctx ) : _app(ctx.app) {}
...
/**
 * The BTCM_DEFINE_PLUGIN() macro will define a btcm::plugin::create_hello_api_plugin()
 * factory method which is expected by the manifest.
 */

BTCM_DEFINE_PLUGIN( hello_api, btcm::example_plugin::hello_api_plugin )
