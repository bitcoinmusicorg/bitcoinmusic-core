...
extended_balance::extended_balance(const account_balance_object& balance) : account_balance_object(balance) {}

class database_api_impl : public std::enable_shared_from_this<database_api_impl>
{
   public:
      explicit database_api_impl( btcm::chain::database& db );
      ~database_api_impl();
...
database_api::database_api( btcm::chain::database& db )
   : my( new database_api_impl( db ) ) {}

database_api::database_api( const btcm::app::api_context& ctx )
   : database_api( *ctx.app.chain_database() ) {}

database_api::~database_api() {}

database_api_impl::database_api_impl( btcm::chain::database& db ):_db(db)
{
...
vector<set<string>> database_api_impl::get_key_references( vector<public_key_type> keys )const
{
   ...
   for( auto& key : keys )
   {
      ...
      const auto& refs = aidx.get_secondary_index<btcm::chain::account_member_index>();
      set<string> result;
...
vector< extended_account > database_api_impl::get_accounts( const vector< string >& names )const
{
   ...
   const auto& proposals_by_account = pidx.get_secondary_index<btcm::chain::required_approval_index>();
   ...
   for( const auto& name: names )
   {
      ...
      results.back().btcm_power = itr->vesting_shares * vesting_price;
...
vector<account_id_type> database_api_impl::get_account_references( account_id_type account_id )const
{
   ...
   return result;*/
   FC_ASSERT( false, "database_api::get_account_references --- Needs to be refactored for btcm." );
}

vector <extended_balance> database_api::get_uia_balances( string account ){
   return my->get_uia_balances(account);
}

vector <extended_balance> database_api_impl::get_uia_balances( string account ){
   const auto& accounts_by_name = _db.get_index_type<account_index>().indices().get<by_name>();
   vector<extended_balance> result;
   ...
   auto bitr = balance_by_account.lower_bound ( std::make_tuple(itr->id, BTCM_SYMBOL ) );
...
vector<content_object> database_api_impl::list_content_by_genre( uint32_t genre, const content_id_type bound, uint16_t limit )const
{
   ...
   const content_by_genre_index& by_genre = idx.get_secondary_index<btcm::chain::content_by_genre_index>();
...
vector<content_object> database_api_impl::list_content_by_category( const string& category, const content_id_type bound, uint16_t limit )const
{
   ...
   result.reserve( limit );
   const auto& idx = _db.get_index_type< primary_index< content_index > >();
   const content_by_category_index& by_category = idx.get_secondary_index<btcm::chain::content_by_category_index>();
...
{
   auto result = trx.get_required_signatures( BTCM_CHAIN_ID,
                                              available_keys,
...
{
   trx.verify_authority( BTCM_CHAIN_ID,
                         [&]( string account_name ){ return &_db.get_account( account_name ).active; },
                         [&]( string account_name ){ return &_db.get_account( account_name ).owner; },
                         [&]( string account_name ){ return &_db.get_account( account_name ).basic; },
                         [&]( string content_url ){ return &_db.get_content( content_url ).manage_master; },
