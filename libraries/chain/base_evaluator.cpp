...
void account_create_with_delegation_evaluator::do_apply( const account_create_with_delegation_operation& o )
{
   ...
   auto target_delegation = asset( wso.median_props.account_creation_fee.amount * BTCM_CREATE_ACCOUNT_DELEGATION_RATIO, BTCM_SYMBOL ) * props.get_vesting_share_price();

   auto current_delegation = asset( o.fee.amount * BTCM_CREATE_ACCOUNT_DELEGATION_RATIO, BTCM_SYMBOL ) * props.get_vesting_share_price() + o.delegation;
   ...
   if( o.delegation.amount > 0 )
   {
      _db.create< vesting_delegation_object >( [&o,&_db]( vesting_delegation_object& vdo )
      {
         ...
         vdo.min_delegation_time = _db.head_block_time() + BTCM_CREATE_ACCOUNT_DELEGATION_TIME;
      });
   }
...
void account_update_evaluator::do_apply( const account_update_operation& o )
{
   ...
   FC_ASSERT( o.account != BTCM_TEMP_ACCOUNT );

   const auto& account = db().get_account( o.account );

   if( o.owner )
   {
      FC_ASSERT( db().head_block_time() - account.last_owner_update > BTCM_OWNER_UPDATE_LIMIT );
...
void transfer_to_vesting_evaluator::do_apply( const transfer_to_vesting_operation& o )
{
   ...
   FC_ASSERT( db().get_balance( from_account, BTCM_SYMBOL) >= o.amount );
   db().adjust_balance( from_account, -o.amount );
...
void withdraw_vesting_evaluator::do_apply( const withdraw_vesting_operation& o )
{
    ...
    if( o.vesting_shares.amount <= 0 ) {
       ...
       db().modify( account, [&o,&now]( account_object& a ) {
         auto new_vesting_withdraw_rate = asset( o.vesting_shares.amount / BTCM_VESTING_WITHDRAW_INTERVALS, VESTS_SYMBOL );
         ...
         a.next_vesting_withdrawal = now + fc::seconds(BTCM_VESTING_WITHDRAW_INTERVAL_SECONDS);
...
void set_withdraw_vesting_route_evaluator::do_apply( const set_withdraw_vesting_route_operation& o )
{
   ...
   if( itr == wd_idx.end() )
   {
      ...
      FC_ASSERT( from_account.withdraw_routes < BTCM_MAX_WITHDRAW_ROUTES );
   ...
   FC_ASSERT( total_percent <= BTCM_100_PERCENT, "More than 100% of vesting allocated to destinations" );
   }
...
void account_witness_proxy_evaluator::do_apply( const account_witness_proxy_operation& o )
{
   ...
   std::array<share_type, BTCM_MAX_PROXY_RECURSION_DEPTH+1> delta;
   delta[0] = -account.vesting_shares.amount;
   for( int i = 0; i < BTCM_MAX_PROXY_RECURSION_DEPTH; ++i )
   ...
   if( o.proxy.size() ) {
      ...
      proxy_chain.reserve( BTCM_MAX_PROXY_RECURSION_DEPTH + 1 );
      ...
      while( cprox->proxy.size() != 0 ) {
         ...
         cprox = &next_proxy;
         FC_ASSERT( proxy_chain.size() <= BTCM_MAX_PROXY_RECURSION_DEPTH, "Proxy chain is too long" );
      }
...
void account_witness_vote_evaluator::do_apply( const account_witness_vote_operation& o )
{
   ...
   if( itr == by_account_witness_idx.end() ) {
      ...
      FC_ASSERT( voter.witnesses_voted_for < BTCM_MAX_ACCOUNT_WITNESS_VOTES, "account has voted for too many witnesses" );
...
void challenge_authority_evaluator::do_apply( const challenge_authority_operation& o )
{
...
#if 0
      FC_ASSERT( challenger.balance >= BTCM_OWNER_CHALLENGE_FEE );
      FC_ASSERT( !challenged.owner_challenged );
      FC_ASSERT( db().head_block_time() - challenged.last_owner_proved > BTCM_OWNER_CHALLENGE_COOLDOWN );

      db().adjust_balance( challenger, - BTCM_OWNER_CHALLENGE_FEE );
      db().create_vesting( db().get_account( o.challenged ), BTCM_OWNER_CHALLENGE_FEE );
  ...
  {
      FC_ASSERT( challenger.balance >= BTCM_ACTIVE_CHALLENGE_FEE );
      FC_ASSERT( !( challenged.owner_challenged || challenged.active_challenged ) );
      FC_ASSERT( db().head_block_time() - challenged.last_active_proved > BTCM_ACTIVE_CHALLENGE_COOLDOWN );

      db().adjust_balance( challenger, - BTCM_ACTIVE_CHALLENGE_FEE );
      db().create_vesting( db().get_account( o.challenged ), BTCM_ACTIVE_CHALLENGE_FEE );
...
void request_account_recovery_evaluator::do_apply( const request_account_recovery_operation& o )
{
   ...
   if( request == recovery_request_idx.end() ) // New Request
   {
      ...
      db().create< account_recovery_request_object >( [&]( account_recovery_request_object& req )
      {
         ...
         req.expires = db().head_block_time() + BTCM_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD;
      });
   }
...
void recover_account_evaluator::do_apply( const recover_account_operation& o )
{
   ...
   FC_ASSERT( db().head_block_time() - account.last_account_recovery > BTCM_OWNER_UPDATE_LIMIT );
...
void change_recovery_account_evaluator::do_apply( const change_recovery_account_operation& o )
{
   ...
   if( request == change_recovery_idx.end() ) // New request
   {
      db().create< change_recovery_account_request_object >( [&]( change_recovery_account_request_object& req )
      {
         ...
         req.effective_on = db().head_block_time() + BTCM_OWNER_AUTH_RECOVERY_PERIOD;
      });
   }
...
void delegate_vesting_shares_evaluator::do_apply( const delegate_vesting_shares_operation& op )
{
   ...
   auto min_delegation = asset( wso.median_props.account_creation_fee.amount * 10, BTCM_SYMBOL ) * gpo.get_vesting_share_price();
       ...
       _db.modify( delegatee, [old_delegation,&deltas,&op] ( account_object& acct ) {
          for( auto& r : acct.redelegations )
          {
             const uint64_t old = ( fc::uint128_t( old_delegation )
                                    * r.second.redelegate_pct / BTCM_100_PERCENT ).to_uint64();
