...
#include <btcm/chain/block_summary_object.hpp>
#include <btcm/chain/compound.hpp>
#include <btcm/chain/database.hpp>
#include <btcm/chain/db_with.hpp>
#include <btcm/chain/exceptions.hpp>
#include <btcm/chain/global_property_object.hpp>
...
#include <btcm/chain/balance_object.hpp>
...
void database::reindex( fc::path data_dir )
{
   try
   {
      ...
      const uint32_t initial_undo_blocks = BTCM_MAX_UNDO_HISTORY;
...
void database::update_account_bandwidth( const account_object& a, uint32_t trx_size ) {
   ...
   if( props.total_vesting_shares.amount > 0 )
   {
      ...
      modify( a, [&props,&now,this,trx_size]( account_object& acnt )
      {
         ...
         uint64_t N = trx_size * BTCM_BANDWIDTH_PRECISION;
         acnt.lifetime_bandwidth += N;
         if( delta_time >= BTCM_BANDWIDTH_AVERAGE_WINDOW_SECONDS )
            acnt.average_bandwidth = N;
         else
         {
            auto old_weight = acnt.average_bandwidth;
            if( delta_time > 0 )
               old_weight = old_weight * (BTCM_BANDWIDTH_AVERAGE_WINDOW_SECONDS - delta_time) / (BTCM_BANDWIDTH_AVERAGE_WINDOW_SECONDS);
...
void database::update_account_market_bandwidth( const account_object& a, uint32_t trx_size )
{
   ...
   if( props.total_vesting_shares.amount > 0 )
   {
      ...
      modify( a, [&props,&now,this,trx_size]( account_object& acnt )
      {
         ...
         {
            auto old_weight = acnt.average_market_bandwidth * (BTCM_BANDWIDTH_AVERAGE_WINDOW_SECONDS - delta_time);
            auto new_weight = delta_time * N;
            acnt.average_market_bandwidth =  (old_weight + new_weight) / (BTCM_BANDWIDTH_AVERAGE_WINDOW_SECONDS);
         }
...
uint32_t database::witness_participation_rate()const
{
   ...
   return uint64_t(BTCM_100_PERCENT) * dpo.recent_slots_filled.popcount() / 128;
}
   ...
   )
{
   ...
   if( witness.running_version != BTCM_BLOCKCHAIN_VERSION )
      pending_block.extensions.insert( block_header_extensions( BTCM_BLOCKCHAIN_VERSION ) );

   const auto& hfp = hardfork_property_id_type()( *this );

   if( hfp.current_hardfork_version < BTCM_BLOCKCHAIN_HARDFORK_VERSION // Binary is newer hardfork than has been applied
   ...
   else if( hfp.current_hardfork_version == BTCM_BLOCKCHAIN_HARDFORK_VERSION // Binary does not know of a new hardfork
      && witness.hardfork_version_vote > BTCM_BLOCKCHAIN_HARDFORK_VERSION ) // Voting for hardfork in the future, that we do not know of...
   {
   ...
   {
      FC_ASSERT( fc::raw::pack_size(pending_block) <= BTCM_MAX_BLOCK_SIZE );
   }
...
void database::pop_block()
{
   try
   {
      ...
      BTCM_ASSERT( head_block.valid(), pop_empty_chain, "there are no blocks to pop" );
...
fc::time_point_sec database::get_slot_time(uint32_t slot_num)const
{
   ...
   auto interval = BTCM_BLOCK_INTERVAL;
   const dynamic_global_property_object& dpo = get_dynamic_global_properties();
...
uint32_t database::get_slot_at_time(fc::time_point_sec when)const
{
   ...
   if( when < first_slot_time )
      return 0;
   return (when - first_slot_time).to_seconds() / BTCM_BLOCK_INTERVAL + 1;
}

/**
 *  Converts BTCM into mbd and adds it to to_account while reducing the BTCM supply
 *  by BTCM and increasing the mbd supply by the specified amount.
 */
asset database::create_mbd(const account_object &to_account, asset btcm)
{
   try
   {
      if( btcm.amount == 0 )
      ...
      {
         adjust_balance( to_account, btcm );
         return btcm;
      }
   }
   FC_CAPTURE_LOG_AND_RETHROW( (to_account.name)(btcm) )
}

/**
 * @param to_account - the account to receive the new vesting shares
 * @param BTCM - BTCM to be converted to vesting shares
 */
asset database::create_vesting( const account_object& to_account, asset btcm )
{
   try
   {
      ...
      /**
       *  The ratio of total_vesting_shares / total_vesting_fund_btcm should not
       ...
       */
      asset new_vesting = btcm * cprops.get_vesting_share_price();
      ...
      modify( cprops, [btcm,new_vesting,sp]( dynamic_global_property_object& props )
      {
         props.total_vesting_fund_btcm += btcm;
   ...
   FC_CAPTURE_AND_RETHROW( (to_account.name)(btcm) )
}
...
bool database::is_voted_streaming_platform(string streaming_platform)const
{
   ...
   for ( auto itr = spidx.begin(); 
         itr != spidx.end() && count < BTCM_MAX_VOTED_STREAMING_PLATFORMS;
         ++itr, ++count )
   {
...
void database::update_witness_schedule4()
{
   ...
   selected_voted.reserve( BTCM_MAX_VOTED_WITNESSES );
   const auto& widx         = get_index_type<witness_index>().indices().get<by_vote_name>();
   for( auto itr = widx.begin();
        itr != widx.end() && selected_voted.size() <  BTCM_MAX_VOTED_WITNESSES;
        ++itr )
   {
   ...
   for( auto witness_count = selected_voted.size();
        sitr != schedule_idx.end() && witness_count < BTCM_MAX_MINERS;
        ++sitr )
   {
   ...
   FC_ASSERT( active_witnesses.size() <= BTCM_MAX_MINERS, "number of active witnesses does not equal BTCM_MAX_MINERS",
                                       ("active_witnesses.size()",active_witnesses.size()) ("BTCM_MAX_MINERS",BTCM_MAX_MINERS) );
...
{
   if( (head_block_num() % BTCM_MAX_MINERS) == 0 )
...
void database::adjust_proxied_witness_votes( const account_object& a,
                                   const std::array< share_type, BTCM_MAX_PROXY_RECURSION_DEPTH+1 >& delta,
                                   int depth )
{
   if( a.proxy != BTCM_PROXY_TO_SELF_ACCOUNT )
   {
      /// nested proxies are not supported, vote will not propagate
      if( depth >= BTCM_MAX_PROXY_RECURSION_DEPTH )
         return;
      ...
      {
         for( int i = BTCM_MAX_PROXY_RECURSION_DEPTH - depth - 1; i >= 0; --i )
         {
   ...
   else
   {
      ...
      for( int i = BTCM_MAX_PROXY_RECURSION_DEPTH - depth; i >= 0; --i )
         total_delta += delta[i];
...
void database::process_vesting_withdrawals()
{
   ...
   while( current != widx.end() && current->next_vesting_withdrawal <= head_block_time() )
   {
      ...
      */
      ...
      share_type vests_deposited_as_btcm = 0;
      share_type vests_deposited_as_vests = 0;

      // Do two passes, the first for vests, the second for btcm. Try to maintain as much accuracy for vests as possible.
           ...
           ++itr )
      {
         if( itr->auto_vest )
         {
            share_type to_deposit = ( ( fc::uint128_t ( to_withdraw.value ) * itr->percent ) / BTCM_100_PERCENT ).to_uint64();
           ...
           ++itr )
      {
         if( !itr->auto_vest )
         {
            ...
            share_type to_deposit = ( ( fc::uint128_t ( to_withdraw.value ) * itr->percent ) / BTCM_100_PERCENT ).to_uint64();
            vests_deposited_as_btcm += to_deposit;
            auto converted_btcm = asset( to_deposit, VESTS_SYMBOL ) * cprops.get_vesting_share_price();

            if( to_deposit > 0 )
            {
               modify( to_account, [&]( account_object& a )
               {
                  a.balance += converted_btcm;
               });

               modify( cprops, [&]( dynamic_global_property_object& o )
               {
                  o.total_vesting_fund_btcm -= converted_btcm;
                  o.total_vesting_shares.amount -= to_deposit;
               });

               push_applied_operation( fill_vesting_withdraw_operation( from_account.name, to_account.name, asset( to_deposit, VESTS_SYMBOL), converted_btcm ) );
            }
         }
      }

      share_type to_convert = to_withdraw - vests_deposited_as_btcm - vests_deposited_as_vests;
      FC_ASSERT( to_convert >= 0, "Deposited more vests than were supposed to be withdrawn" );

      auto converted_btcm = asset( to_convert, VESTS_SYMBOL ) * cprops.get_vesting_share_price();
      ...
      modify( from_account, [&]( account_object& a )
      {
         ...
         {
            a.next_vesting_withdrawal += fc::seconds( BTCM_VESTING_WITHDRAW_INTERVAL_SECONDS );
         }
      });
      ...
      push_applied_operation( fill_vesting_withdraw_operation( from_account.name, from_account.name, asset( to_convert, VESTS_SYMBOL ), converted_btcm ) );
   }
}
                               ...
                               const flat_map<streaming_platform_id_type, sp_helper> platforms )
{
   ...
   for( const auto& sph : platforms )
   {
      ...
      for ( const auto& listened : sph.second.account_listening_times )
      {
         ...
         db.modify( consumer, [&listened,&sph] ( account_object& a ) {
      ...
      for ( const auto& listened : sph.second.user_listening_times )
      {
         ...
         if( consumer.total_listening_time == listened.second )
            ...
            db.modify( consumer, [&listened] ( streaming_platform_user_object& a ) {
               a.total_listening_time -= listened.second;
            });
...
{ try {
   ...
   asset total_payout = content_reward;
   ...
   while ( itr != ridx.end() && itr->created <= cashing_time )
   {
      ...
      {
         reporter_reward = asset( platform_reward.amount * *itr->reward_pct / BTCM_100_PERCENT,
...
{try{
   ...
   else
   {
      ...
      for ( const auto& di : co.distributions_master )
      {
         ...
         auto mbd_btcm     = author_reward;
         auto vesting_btcm = author_reward - mbd_btcm;

         const auto& author = get_account( di.payee );
         auto vest_created = create_vesting( author, vesting_btcm );
         auto mbd_created = create_mbd( author, mbd_btcm );
...
{try{
   ...
   auto mbd_btcm     = asset(0, BTCM_SYMBOL);
   auto vesting_btcm = payout - mbd_btcm;
   auto vest_created = create_vesting( owner, vesting_btcm );
   auto mbd_created = create_mbd( owner, mbd_btcm );
...
{try{
   ...
   platform_reward.amount = platform_reward.amount * content.playing_reward / BTCM_100_PERCENT;

   payout.amount -= platform_reward.amount;
   asset comp_reward = payout;
   comp_reward.amount = comp_reward.amount * content.publishers_share / BTCM_100_PERCENT;
...
/**
 *  Overall the network has an inflation rate of 9.5% of virtual btcm per year
...
void database::process_funds( const asset& content_reward, const asset& witness_pay, const asset& vesting_reward )
{
   ...
   {
       p.total_vesting_fund_btcm += vesting_reward;
       p.total_reward_fund_btcm  += content_reward ;
...
{
   const auto initial_content_allocation = content_reward;
   ...
   {
        p.total_reward_fund_btcm  -= delta;
...
asset database::get_content_reward()const
{
   ...
   static_assert( BTCM_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
   const auto amount = calc_percent_reward_per_day_0_2< BTCM_CONTENT_APR_PERCENT_0_2 >( props.virtual_supply.amount );
   return std::max( asset( amount, BTCM_SYMBOL ), BTCM_MIN_CONTENT_REWARD );
}

asset database::get_vesting_reward()const
{
   FC_ASSERT(false, "Unused - fixed to 0");
   const auto& props = get_dynamic_global_properties();
   static_assert( BTCM_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
   const auto amount = calc_percent_reward_per_block_0_2< BTCM_VESTING_ARP_PERCENT_0_2 >( props.virtual_supply.amount );
   return asset( amount, BTCM_SYMBOL );
}
...
asset database::get_producer_reward()
{
   ...
   //const auto amount = calc_percent_reward_per_block_0_2< BTCM_PRODUCER_APR_PERCENT_0_2 >( props.virtual_supply.amount);
   const auto pay = BTCM_MINING_REWARD;
   const auto& witness_account = get_account( props.current_witness );

   /// pay witness in vesting shares
   if( props.head_block_number >= BTCM_START_MINER_VOTING_BLOCK || (witness_account.vesting_shares.amount.value == 0) )
   {
...
void database::process_conversions()
{
   ...
   {
       p.current_supply += net_btcm;
       p.current_mbd_supply -= net_mbd;
       p.virtual_supply += net_btcm;
       p.virtual_supply -= net_mbd * get_feed_history().effective_median_history;
   } );
}

asset database::to_mbd( const asset& btcm )const
{
...
void database::account_recovery_processing()
{
   ...
   while( hist != hist_idx.end() && time_point_sec( hist->last_valid_time + BTCM_OWNER_AUTH_RECOVERY_PERIOD ) < head_block_time() )
   {
      remove( *hist );
      hist = hist_idx.begin();
   }
...
{
   reset_indexes();
   _undo_db.set_max_size( BTCM_MIN_UNDO_HISTORY );
...
void database::init_genesis( const genesis_state_type& initial_allocation )
{
   try
   {
      ...
      for( int i = 0; i < BTCM_NUM_INIT_MINERS; ++i )
      {
         create<account_object>([&](account_object& a)
         {
            a.name = BTCM_INIT_MINER_NAME + ( i ? fc::to_string( i ) : std::string() );
            a.owner.weight_threshold = 1;
            a.owner.add_authority( init_public_key, 1 );
            a.active  = a.owner;
            a.basic = a.active;
            a.memo_key = init_public_key;
            a.balance  = asset( i ? 0 : initial_allocation.init_supply, BTCM_SYMBOL );
         } );

         create<witness_object>([&](witness_object& w )
         {
            w.owner        = BTCM_INIT_MINER_NAME + ( i ? fc::to_string(i) : std::string() );
      ...
      {
         p.current_witness = BTCM_INIT_MINER_NAME;
         p.time = BTCM_GENESIS_TIME;
         p.recent_slots_filled = fc::uint128::max_value();
         p.participation_count = 128;
         p.current_supply = asset( initial_allocation.init_supply, BTCM_SYMBOL );
         p.virtual_supply = p.current_supply;
         p.maximum_block_size = BTCM_MAX_BLOCK_SIZE;
      } );

      //Create core assets
      const asset_object& btcm_asset = create<asset_object>( []( asset_object& a )
      {
         a.current_supply = 0;
         a.precision = BTCM_ASSET_PRECISION;
         a.symbol_string = "BTCM";
         a.options.max_supply = BTCM_MAX_SHARE_SUPPLY;
         a.options.description = "BTCM Core asset";
      });
      create<asset_object>( []( asset_object& a )
      {
         ...
         a.options.max_supply = BTCM_MAX_SHARE_SUPPLY;
         a.options.description = "BTCM Power asset";
      });
      create<asset_object>( []( asset_object& a )
      {
         a.current_supply = 0;
      ...
      {
         hpo.processed_hardforks.push_back( BTCM_GENESIS_TIME );
      } );

      // Create witness scheduler
      create<witness_schedule_object>([&]( witness_schedule_object& wso )
      {
         wso.current_shuffled_witnesses.emplace_back(BTCM_INIT_MINER_NAME);
         for( int i = 0; i < BTCM_NUM_INIT_MINERS; ++i )
            wso.current_shuffled_witnesses.emplace_back( BTCM_INIT_MINER_NAME + ( i ? fc::to_string( i ) : std::string() ) );
      } );
      ...
      for( const auto& handout : initial_allocation.initial_balances )
      {
         ...
         const asset amount( handout.amount, BTCM_SYMBOL );
         try {
            const account_object& owner = get_account( handout.owner );
            adjust_balance( owner, amount );
         } catch (const fc::assert_exception& e) {
            create<balance_object>([&amount,&handout](balance_object& b){
                 b.owner = address( handout.owner );
                 b.balance = amount;
            });
         }

         modify( gpo, [&amount]( dynamic_global_property_object& p )
         {
            p.current_supply += amount;
            p.virtual_supply = p.current_supply;
         } );

         modify( btcm_asset, [&amount]( asset_object& a ){
            a.current_supply += amount.amount;
         });
      }
...
{ try {
   ...
   const auto vesting_reward = asset( 0, BTCM_SYMBOL );
...
try {
   if( (head_block_num() % BTCM_FEED_INTERVAL_BLOCKS) != 0 )
      return;
   ...
   for( const auto& w : wso.current_shuffled_witnesses ) {
      ...
      if( wit.last_mbd_exchange_update < now + BTCM_MAX_FEED_AGE &&
   ...
   if( feeds.size() >= BTCM_MIN_FEEDS ) {
      std::sort( feeds.begin(), feeds.end() );
      auto median_feed = feeds[feeds.size()/2];

      modify( get_feed_history(), [&]( feed_history_object& fho ){
         fho.price_history.push_back( median_feed );
         if( fho.price_history.size() > BTCM_FEED_HISTORY_WINDOW )
...
{ try {
   ...
   const chain_id_type& chain_id = BTCM_CHAIN_ID;
   auto trx_id = trx.id();
   ...
   if( BOOST_LIKELY(head_block_num() > 0) )
   {
      ...
      FC_ASSERT( trx.expiration <= now + fc::seconds(BTCM_MAX_TIME_UNTIL_EXPIRATION), "",
                 ("trx.expiration",trx.expiration)("now",now)("max_til_exp",BTCM_MAX_TIME_UNTIL_EXPIRATION));
...
void database::update_global_dynamic_data( const signed_block& b )
{
   ...
   if( head_block_time() != fc::time_point_sec() )
   {
      ...
      for( uint32_t i = 0; i < missed_blocks; ++i )
      {
         ...
         if( witness_missed.owner != b.witness )
         {
            ...
            {
               w.total_missed++;
               if( head_block_num() - w.last_confirmed_block_num  > BTCM_BLOCKS_PER_DAY )
   ...
   modify( _dgp, [&]( dynamic_global_property_object& dgp )
   {
      ...
      if( dgp.head_block_number % 20 == 0 )
      {
         ...
         if( dgp.current_reserve_ratio > BTCM_MAX_RESERVE_RATIO )
            dgp.current_reserve_ratio = BTCM_MAX_RESERVE_RATIO;
      }
      dgp.max_virtual_bandwidth = (dgp.maximum_block_size * dgp.current_reserve_ratio *
                                  BTCM_BANDWIDTH_PRECISION * BTCM_BANDWIDTH_AVERAGE_WINDOW_SECONDS) / BTCM_BLOCK_INTERVAL;
   } );

   if( !(get_node_properties().skip_flags & skip_undo_history_check) )
   {
      BTCM_ASSERT( _dgp.head_block_number - _dgp.last_irreversible_block_num  < BTCM_MAX_UNDO_HISTORY, undo_database_exception,
                 "Please add a checkpoint if you would like to continue applying blocks beyond this point.",
                 ("last_irreversible_block_num",_dgp.last_irreversible_block_num)("head", _dgp.head_block_number)
                 ("max_undo",BTCM_MAX_UNDO_HISTORY) );
   }
...
void database::update_virtual_supply()
{
   modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& dgp )
   {
      dgp.virtual_supply = dgp.current_supply
         + ( get_feed_history().effective_median_history.is_null() ? asset( 0, BTCM_SYMBOL )
...
public:
   ...
   {
      FC_ASSERT( counter < BTCM_MAX_MINERS * 2, "Max proposal nesting depth exceeded!" );
      counter++;
   }
...
void database::update_last_irreversible_block()
{
    ...
    *
    */
   if( head_block_num() < BTCM_START_MINER_VOTING_BLOCK )
   {
      modify( dpo, [&]( dynamic_global_property_object& _dpo )
      {
         if ( head_block_num() > BTCM_MAX_MINERS )
            _dpo.last_irreversible_block_num = head_block_num() - BTCM_MAX_MINERS;
      } );
      return;
   }
   ...
   static_assert( BTCM_IRREVERSIBLE_THRESHOLD > 0, "irreversible threshold must be nonzero" );

   // 1 1 1 2 2 2 2 2 2 2 -> 2     .7*10 = 7
   // 1 1 1 1 1 1 1 2 2 2 -> 1
   // 3 3 3 3 3 3 3 3 3 3 -> 3

   size_t offset = ((BTCM_100_PERCENT - BTCM_IRREVERSIBLE_THRESHOLD) * wit_objs.size() / BTCM_100_PERCENT);
      ...
      modify( a, [&]( account_object& acnt )   
      {
         ...
         if(delta.asset_id==XUSD_SYMBOL)
         {
            if( a.mbd_seconds_last_update != head_block_time() )
            {
               ...
               if( acnt.mbd_seconds > 0 && (acnt.mbd_seconds_last_update - acnt.mbd_last_interest_payment).to_seconds() > BTCM_SBD_INTEREST_COMPOUND_INTERVAL_SEC )
               {
...
void database::adjust_supply( const asset& delta, bool adjust_vesting )
{
   ...
   const auto& props = get_dynamic_global_properties();
   if( props.head_block_number < BTCM_BLOCKS_PER_DAY*7 )
      adjust_vesting = false;

   modify( props, [&]( dynamic_global_property_object& props )
   {
      if(delta.asset_id == BTCM_SYMBOL){
         asset new_vesting( (adjust_vesting && delta.amount > 0) ? delta.amount * 9 : 0, BTCM_SYMBOL );
...
void database::init_hardforks()
{
   ...
   FC_ASSERT( hardforks.last_hardfork <= BTCM_NUM_HARDFORKS, "Chain knows of more hardforks than configuration",
              ("hardforks.last_hardfork",hardforks.last_hardfork)("BTCM_NUM_HARDFORKS",BTCM_NUM_HARDFORKS) );
   FC_ASSERT( _hardfork_versions[ hardforks.last_hardfork ] <= BTCM_BLOCKCHAIN_VERSION, "Blockchain version is older than last applied hardfork" );
}
...
void database::process_hardforks()
{
   try
   {
      ...
      {
         if( hardforks.last_hardfork < BTCM_NUM_HARDFORKS )
...
void database::set_hardfork( uint32_t hardfork, bool apply_now )
{
   ...
   for( uint32_t i = hardforks.last_hardfork + 1; i <= hardfork && i <= BTCM_NUM_HARDFORKS; i++ )
   {
...
void database::validate_invariants()const
{
   try
   {
      ...
      for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
      {
         ...
         total_vsf_votes += ( itr->proxy == BTCM_PROXY_TO_SELF_ACCOUNT ?
                                 itr->witness_vote_weight() :
                                 ( BTCM_MAX_PROXY_RECURSION_DEPTH > 0 ?
                                      itr->proxied_vsf_votes[BTCM_MAX_PROXY_RECURSION_DEPTH - 1] :
      ...
      {
         if( itr->sell_price.base.asset_id == BTCM_SYMBOL )
         {
      ...
      total_supply += gpo.total_vesting_fund_btcm;
...
{
   //TODO_BTCM - recalculate score here
   try
   {
      ...
      for( const auto& account : get_index_type<account_index>().indices() )
      {
         modify( account, [&]( account_object& a )
         {
            ...
            a.vesting_withdraw_rate  = asset( a.to_withdraw / BTCM_VESTING_WITHDRAW_INTERVALS, VESTS_SYMBOL );
            if( a.vesting_withdraw_rate.amount == 0 )
               a.vesting_withdraw_rate.amount = 1;

            for( uint32_t i = 0; i < BTCM_MAX_PROXY_RECURSION_DEPTH; ++i )
...
void database::retally_witness_vote_counts( bool force )
{
   ...
   for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
   {
      ...
      if( force || (a.proxy != BTCM_PROXY_TO_SELF_ACCOUNT  ) )
      {
        const auto& vidx = get_index_type<witness_vote_index>().indices().get<by_account_witness>();
...
uint64_t database::get_scoring(const account_object& ao ) const
{
   ...
   for ( const auto & a : ao.friends ) {
      ...
      score += detail::isqrt(f.get_scoring_vesting()) * BTCM_1ST_LEVEL_SCORING_PERCENTAGE / 100;
   }
   for ( const auto & a : ao.second_level ){
      const auto& sl = get<account_object>( a );
      score += detail::isqrt(sl.get_scoring_vesting()) * BTCM_2ST_LEVEL_SCORING_PERCENTAGE / 100;
   }
   return score;
}
...
void database::recursive_recalculate_score(const account_object& a, share_type delta)
{
   ...
   for( auto &f:a.friends ) {
      ...
      modify<account_object>(f_object,[score_delta](account_object& ao){
           ao.score += score_delta * BTCM_1ST_LEVEL_SCORING_PERCENTAGE / 100;
      });
   }
   ...
   for( auto &f:a.second_level ) {
      const auto& f_object = get<account_object>(f);
      modify<account_object>(f_object,[score_delta](account_object& ao){
           ao.score += score_delta * BTCM_2ST_LEVEL_SCORING_PERCENTAGE / 100;
      });
   }
}
...
void database::recalculate_score(const account_object& a) {
   ...
   for( auto &f:a.friends ) {
      const auto& f_object = get<account_object>(f);
      score += detail::isqrt(f_object.get_scoring_vesting()) * BTCM_1ST_LEVEL_SCORING_PERCENTAGE / 100;
   }

   for( auto &f:a.second_level ) {
      const auto& f_object = get<account_object>(f);
      score += detail::isqrt(f_object.get_scoring_vesting()) * BTCM_2ST_LEVEL_SCORING_PERCENTAGE / 100;
   }
