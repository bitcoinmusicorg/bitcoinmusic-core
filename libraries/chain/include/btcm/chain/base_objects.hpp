...
namespace btcm { namespace chain {
   ...
   /**
    *  This object is used to track pending requests to convert mbd to btcm
    */
    ...
    *  1000 BTCM or 1000 * virtual_supply / (100*blocks_per_year) aka 10 * virtual_supply / blocks_per_year
    *
   ...
   class liquidity_reward_balance_object : public abstract_object<liquidity_reward_balance_object> {
      public:
         ...
         int64_t         btcm_volume = 0;
         int64_t         mbd_volume = 0;
         uint128_t       weight = 0;

         /// this is the sort index
         uint128_t volume_weight()const { 
            return btcm_volume * mbd_volume * is_positive(); 
        }
         uint128_t min_volume_weight()const { 
            return std::min(btcm_volume,mbd_volume) * is_positive(); 
        }
         void update_weight( bool hf9 ) {
             weight = hf9 ? min_volume_weight() : volume_weight();
         }

         inline int is_positive()const { return ( btcm_volume > 0 && mbd_volume > 0 ) ? 1 : 0; }
   ...
   class feed_history_object  : public abstract_object<feed_history_object> {
      public:
         ...
         price               previous_actual_median; ///< the actual median of the price history as of one hour ago
         price               effective_median_history; ///< the base converting XUSD back to BTCM
         price               previous_effective_median; ///< the base converting XUSD back to BTCM, as of one hour ago
...
FC_REFLECT_DERIVED( btcm::chain::limit_order_object, (graphene::db::object),
                    (created)(expiration)(seller)(orderid)(for_sale)(sell_price) )

FC_REFLECT_DERIVED( btcm::chain::feed_history_object, (graphene::db::object),
                    (actual_median_history)(effective_median_history)(price_history) )

FC_REFLECT_DERIVED( btcm::chain::convert_request_object, (graphene::db::object),
                    (owner)(requestid)(amount)(conversion_date) )

FC_REFLECT_DERIVED( btcm::chain::liquidity_reward_balance_object, (graphene::db::object),
                    (owner)(btcm_volume)(mbd_volume)(weight)(last_update) )

FC_REFLECT_DERIVED( btcm::chain::withdraw_vesting_route_object, (graphene::db::object),
                    (from_account)(to_account)(percent)(auto_vest) )

FC_REFLECT_DERIVED( btcm::chain::escrow_object, (graphene::db::object), 
