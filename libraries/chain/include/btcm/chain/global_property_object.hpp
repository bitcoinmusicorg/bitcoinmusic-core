...
namespace btcm { namespace chain {
   ...
   class dynamic_global_property_object : public abstract_object<dynamic_global_property_object>
   {
      public:
         ...
         asset       supply_delta               = asset( 0, BTCM_SYMBOL );


         uint32_t    maximum_proposal_lifetime = 86400;
         price       get_vesting_share_price() const
         {
            if ( total_vesting_fund_btcm.amount == 0 || total_vesting_shares.amount == 0 )
               return price ( asset( 1, BTCM_SYMBOL ), asset( 1, VESTS_SYMBOL ) );

            return price( total_vesting_shares, total_vesting_fund_btcm );
         }
          ...
          *
          *    max_bandwidth = maximum_block_size * BTCM_BANDWIDTH_AVERAGE_WINDOW_SECONDS / BTCM_BLOCK_INTERVAL
          *
          ...
          *   reaches BTCM_MAX_RESERVE_RATIO.  Any time average_block_size is greater than
          *   50% it falls by 1%.  Upward adjustments happen once per round, downward adjustments
          *   happen every block.
          */
         uint64_t current_reserve_ratio = 1;

         uint32_t delegation_return_period = BTCM_DELEGATION_RETURN_PERIOD;
...
FC_REFLECT_DERIVED( btcm::chain::dynamic_global_property_object, (graphene::db::object),
                    (head_block_number)
                    (head_block_id)
                    (time)
                    (current_witness)
                    ...
                    (total_vesting_fund_btcm)
                    (total_vesting_shares)
                    (total_reward_fund_btcm)
                    (supply_delta)
