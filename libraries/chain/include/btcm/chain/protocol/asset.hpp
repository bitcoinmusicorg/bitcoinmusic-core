...
namespace btcm { namespace chain {
   ...
   struct asset
   {
      ...
      asset( share_type a = 0, asset_id_type id = BTCM_SYMBOL )
...
FC_REFLECT( btcm::chain::asset, (amount)(asset_id) )
FC_REFLECT( btcm::chain::price, (base)(quote) )
...
namespace fc {
    inline void to_variant( const btcm::chain::asset& var,  fc::variant& vo, uint32_t max_depth = 1 )
    {
       ...
       else
       {
          ...
          fc::reflector<btcm::chain::asset>::visit( fc::to_variant_visitor<btcm::chain::asset>( mvo, var, max_depth ) );
          vo = std::move(mvo);
       }
           
    }
    inline void from_variant( const fc::variant& var,  btcm::chain::asset& vo, uint32_t max_depth = 1 )
    {
       if (var.is_object())
          fc::reflector<btcm::chain::asset>::visit( from_variant_visitor<btcm::chain::asset>( var.get_object(), vo, max_depth ) );
       else
       {
