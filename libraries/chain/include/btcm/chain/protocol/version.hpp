...
/*
 * This class represents the basic versioning scheme of the Bitcoin Music blockchain.
...
namespace fc
{
   void to_variant( const btcm::chain::version& v, variant& var, uint32_t max_depth = 1 );
   void from_variant( const variant& var, btcm::chain::version& v, uint32_t max_depth = 1 );

   void to_variant( const btcm::chain::hardfork_version& hv, variant& var, uint32_t max_depth = 1 );
   void from_variant( const variant& var, btcm::chain::hardfork_version& hv, uint32_t max_depth = 1 );
} // fc

#include <fc/reflect/reflect.hpp>
FC_REFLECT( btcm::chain::version, (v_num) )
FC_REFLECT_DERIVED( btcm::chain::hardfork_version, (btcm::chain::version), )

FC_REFLECT( btcm::chain::hardfork_version_vote, (hf_version)(hf_time) )
