...
namespace btcm { namespace chain {
      ...
      asset asset::from_string(const string& from) {
         ...
         if(space_pos < dot_pos) { //no dot in the first part
            ...
            std::string fractpart = s.substr( dot_pos+1, std::min<size_t>(space_pos-dot_pos-1, BTCM_ASSET_PRECISION));
            while (fractpart.size() < BTCM_ASSET_PRECISION)
               fractpart+='0';
      ...
      price price::max( asset_id_type base, asset_id_type quote ) { return asset( share_type(BTCM_MAX_SHARE_SUPPLY), base ) / asset( share_type(1), quote); }
      price price::min( asset_id_type base, asset_id_type quote ) { return asset( 1, base ) / asset( BTCM_MAX_SHARE_SUPPLY, quote); }
