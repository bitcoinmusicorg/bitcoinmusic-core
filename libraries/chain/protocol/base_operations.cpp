...
namespace btcm { namespace chain {
   ...
   void account_create_operation::validate() const
   {
      ...
      FC_ASSERT( is_asset_type( fee, BTCM_SYMBOL ), "Account creation fee must be BTCM" );
      owner.validate();
      active.validate();
      basic.validate();
      ...
      FC_ASSERT( fee >= asset( 0, BTCM_SYMBOL ) );
   }
   ...
   void account_create_with_delegation_operation::validate() const
   {
      ...
      FC_ASSERT( fee >= asset( 0, BTCM_SYMBOL ), "Account creation fee cannot be negative" );
      FC_ASSERT( delegation >= asset( 0, VESTS_SYMBOL ), "Delegation cannot be negative" );
   }

   void account_update_operation::validate() const
   {
      FC_ASSERT( account != BTCM_TEMP_ACCOUNT );
   ...
   { try {
      ...
      FC_ASSERT( memo.size() < BTCM_MAX_MEMO_SIZE, "Memo is too large" );
   ...
   void transfer_to_vesting_operation::validate() const
   {
      ...
      FC_ASSERT( is_asset_type( amount, BTCM_SYMBOL ), "Amount must be BTCM" );
      if ( !to.empty() ) FC_ASSERT( is_valid_account_name( to ), "To account name invalid" );
      FC_ASSERT( amount > asset( 0, BTCM_SYMBOL ), "Must transfer a nonzero amount" );
   }
   ...
   void set_withdraw_vesting_route_operation::validate() const
   {
      ...
      FC_ASSERT( 0 <= percent && percent <= BTCM_100_PERCENT, "Percent must be valid btcm percent" );
   }
   ...
   void witness_update_operation::validate() const
   {
      ...
      FC_ASSERT( url.size() <= BTCM_MAX_WITNESS_URL_LENGTH );
