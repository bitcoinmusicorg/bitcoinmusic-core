...
btcm::chain::transaction_id_type btcm::chain::transaction::id() const
{
   auto h = digest();
...
const signature_type& btcm::chain::signed_transaction::sign(const private_key_type& key, const chain_id_type& chain_id)
{
...
signature_type btcm::chain::signed_transaction::sign(const private_key_type& key, const chain_id_type& chain_id)const
{
   digest_type::encoder enc;
   fc::raw::pack( enc, chain_id );
   fc::raw::pack( enc, *this );
...
{ try {
   ...
   {
      BTCM_ASSERT( s.check_authority(&auth), tx_missing_other_auth, "Missing Authority", ("auth",auth)("sigs",sigs) );
   }

   // fetch all of the top level authorities
   for( auto id : required_basic )
   {
      BTCM_ASSERT( s.check_authority(id) ||
   ...
   {
      BTCM_ASSERT( s.check_authority(get_master_content(con)),
   ...
   {
      BTCM_ASSERT( s.check_authority(get_comp_content(con)),
                       tx_missing_active_auth, "Missing Content Authority for content ${con}", ("con", con)("auth",*get_comp_content(con)));
   }

   for( auto id : required_owner )
   {
      BTCM_ASSERT( owner_approvals.find(id) != owner_approvals.end() ||
                       s.check_authority(get_owner(id)),
   ...
   ) const
{
   ...
   for( const public_key_type& k : s )
   {
      ...
      try
      {
         btcm::chain::verify_authority( operations, result, get_active,
			                get_owner, get_basic, get_master_content,
				        get_comp_content, false, max_recursion );
...
{ try {
   btcm::chain::verify_authority( operations, get_signature_keys( chain_id ),
