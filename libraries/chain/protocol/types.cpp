...
namespace btcm { namespace chain {
    ...
    public_key_type::public_key_type( const std::string& base58str )
    {
      ...
      //        to make public_key_type API more similar to address API
       std::string prefix( BTCM_ADDRESS_PREFIX );
    ...
    public_key_type::operator std::string() const
    {
       ...
       return BTCM_ADDRESS_PREFIX + fc::to_base58( data.data(), data.size() );
    }
...
{
    using namespace std;
    void to_variant( const btcm::chain::public_key_type& var, fc::variant& vo, uint32_t max_depth )
    {
        vo = std::string( var );
    }

    void from_variant( const fc::variant& var, btcm::chain::public_key_type& vo, uint32_t max_depth )
    {
        vo = btcm::chain::public_key_type( var.as_string() );
    }

    void to_variant( const btcm::chain::extended_public_key_type& var, fc::variant& vo, uint32_t max_depth )
    {
       vo = std::string( var );
    }

    void from_variant( const fc::variant& var, btcm::chain::extended_public_key_type& vo, uint32_t max_depth )
    {
       vo = btcm::chain::extended_public_key_type( var.as_string() );
    }

    void to_variant( const btcm::chain::extended_private_key_type& var, fc::variant& vo, uint32_t max_depth )
    {
       vo = std::string( var );
    }

    void from_variant( const fc::variant& var, btcm::chain::extended_private_key_type& vo, uint32_t max_depth )
    {
       vo = btcm::chain::extended_private_key_type( var.as_string() );
    }
