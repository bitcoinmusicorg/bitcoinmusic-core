...
namespace fc {
   ...
   template<typename Stream, typename K, typename V>
    ...
    template<typename T>
    inline std::vector<char> pack_to_vector( const T& v, uint32_t _max_depth ) {
       FC_ASSERT( _max_depth > 0 );
       --_max_depth;
       datastream<size_t> ps;
    ...
    template<typename T>
    inline T unpack_from_vector( const std::vector<char>& s, uint32_t _max_depth )
    { try  {
       FC_ASSERT( _max_depth > 0 );
       T tmp;
       if( s.size() ) {
    ...
    template<typename T>
    inline void unpack_from_vector( const std::vector<char>& s, T& tmp, uint32_t _max_depth )
    { try  {
       FC_ASSERT( _max_depth > 0 );
       if( s.size() ) {
          datastream<const char*>  ds( s.data(), size_t(s.size()) );
