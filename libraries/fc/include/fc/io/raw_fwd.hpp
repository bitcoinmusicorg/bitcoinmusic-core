...
namespace fc {
   ...
   namespace raw {
    ...
    template<typename T> inline std::vector<char> pack_to_vector( const T& v, uint32_t _max_depth=FC_PACK_MAX_DEPTH );
    template<typename T> inline T unpack_from_vector( const std::vector<char>& s, uint32_t _max_depth=FC_PACK_MAX_DEPTH );
    template<typename T> inline T unpack_from_char_array( const char* d, uint32_t s, uint32_t _max_depth=FC_PACK_MAX_DEPTH );
    template<typename T> inline void unpack_from_char_array( const char* d, uint32_t s, T& v, uint32_t _max_depth=FC_PACK_MAX_DEPTH );
} }
