...
#include <btcm/chain/block_database.hpp>
#include <btcm/chain/database.hpp>
#include <btcm/chain/witness_objects.hpp>

#include <graphene/utilities/key_conversion.hpp>

#include <btcm/plugins/debug_node/debug_node_api.hpp>
#include <btcm/plugins/debug_node/debug_node_plugin.hpp>

namespace btcm { namespace plugin { namespace debug_node {
...
{
   public:
      debug_node_api_impl( btcm::app::application& _app );
      ...
      fc::optional< btcm::chain::signed_block > debug_pop_block();
      //void debug_push_block( const btcm::chain::signed_block& block );
      btcm::chain::witness_schedule_object debug_get_witness_schedule();
      btcm::chain::hardfork_property_object debug_get_hardfork_property_object();
      ...
      std::shared_ptr< btcm::plugin::debug_node::debug_node_plugin > get_plugin();

      btcm::app::application& app;
};

debug_node_api_impl::debug_node_api_impl( btcm::app::application& _app ) : app( _app )
{}
...
uint32_t debug_node_api_impl::debug_push_blocks( const std::string& src_filename, uint32_t count, bool skip_validate_invariants )
{
   ...
   std::shared_ptr< btcm::chain::database > db = app.chain_database();
   ...
   if( fc::is_directory( src_path ) )
   {
      ...
      btcm::chain::block_database bdb;
      bdb.open( src_path );
      uint32_t first_block = db->head_block_num()+1;
      uint32_t skip_flags = btcm::chain::database::skip_nothing;
      if( skip_validate_invariants )
         skip_flags = skip_flags | btcm::chain::database::skip_validate_invariants;
      for( uint32_t i=0; i<count; i++ )
      {
         fc::optional< btcm::chain::signed_block > block = bdb.fetch_by_number( first_block+i );
         if( !block.valid() )
         {
...
uint32_t debug_node_api_impl::debug_generate_blocks( const std::string& debug_key, uint32_t count )
{
   ...
   btcm::chain::public_key_type debug_public_key = debug_private_key->get_public_key();
   ...
   for( uint32_t i=0; i<count; i++ )
   {
      ...
      btcm::chain::public_key_type scheduled_key = scheduled_witness.signing_key;
      ...
      db->generate_block( scheduled_time, scheduled_witness_name, *debug_private_key, btcm::chain::database::skip_nothing );
   }
...
uint32_t debug_node_api_impl::debug_generate_blocks_until( const std::string& debug_key, const fc::time_point_sec& head_block_time, bool generate_sparsely )
{
   ...
   if( generate_sparsely )
   {
      ...
      FC_ASSERT( head_block_time.sec_since_epoch() - db->head_block_time().sec_since_epoch() < BTCM_BLOCK_INTERVAL, "", ("desired_time", head_block_time)("db->head_block_time()",db->head_block_time()) );
   }
   else
   {
...
fc::optional< btcm::chain::signed_block > debug_node_api_impl::debug_pop_block()
{
   std::shared_ptr< btcm::chain::database > db = app.chain_database();
   return db->fetch_block_by_number( db->head_block_num() );
}

/*void debug_node_api_impl::debug_push_block( const btcm::chain::signed_block& block )
{
   app.chain_database()->push_block( block );
}*/

btcm::chain::witness_schedule_object debug_node_api_impl::debug_get_witness_schedule()
{
   return btcm::chain::witness_schedule_id_type()( *app.chain_database() );
}

btcm::chain::hardfork_property_object debug_node_api_impl::debug_get_hardfork_property_object()
{
   return btcm::chain::hardfork_property_id_type()( *app.chain_database() );
}
...
std::shared_ptr< btcm::plugin::debug_node::debug_node_plugin > debug_node_api_impl::get_plugin()
{
...
void debug_node_api_impl::debug_set_hardfork( uint32_t hardfork_id )
{
   ...
   if( hardfork_id > BTCM_NUM_HARDFORKS )
      return;

   fc::limited_mutable_variant_object update( GRAPHENE_MAX_NESTED_OBJECTS );
   auto hfp_id = btcm::chain::hardfork_property_id_type();
...
{
   idump( (btcm::chain::hardfork_property_id_type()( *app.chain_database() ))(hardfork_id) );
   return btcm::chain::hardfork_property_id_type()( *app.chain_database() ).last_hardfork >= hardfork_id;
}

} // detail

debug_node_api::debug_node_api( const btcm::app::api_context& ctx )
{
...
fc::optional< btcm::chain::signed_block > debug_node_api::debug_pop_block()
{
   return my->debug_pop_block();
}

/*void debug_node_api::debug_push_block( btcm::chain::signed_block& block )
{
   my->debug_push_block( block );
}*/

btcm::chain::witness_schedule_object debug_node_api::debug_get_witness_schedule()
{
   return my->debug_get_witness_schedule();
}

btcm::chain::hardfork_property_object debug_node_api::debug_get_hardfork_property_object()
{
