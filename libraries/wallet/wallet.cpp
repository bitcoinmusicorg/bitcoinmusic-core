...
#include <btcm/wallet/wallet.hpp>
#include <btcm/wallet/api_documentation.hpp>
#include <btcm/wallet/reflect_util.hpp>
...
namespace btcm { namespace wallet {
...
public:
   ...
   variant_object about() const
   {
      ...
      result["btcm_revision"]            = graphene::utilities::git_revision_sha;
      result["btcm_revision_age"]        = fc::get_approximate_relative_time_string( fc::time_point_sec( graphene::utilities::git_revision_unix_timestamp ) );
   ...
   bool import_key(string wif_key)
   {
      ...
      btcm::chain::public_key_type wif_pub_key = optional_private_key->get_public_key();
   ...
   int find_first_unused_derived_key_index(const fc::ecc::private_key& parent_key)
   {
      ...
      for (int key_index = 0; ; ++key_index)
      {
         ...
         btcm::chain::public_key_type derived_public_key = derived_private_key.get_public_key();
   ...
   { try {
         ...
         btcm::chain::public_key_type owner_pubkey = owner_privkey.get_public_key();
         btcm::chain::public_key_type active_pubkey = active_privkey.get_public_key();
         btcm::chain::public_key_type memo_pubkey = memo_privkey.get_public_key();
   ...
   {
      FC_ASSERT( tx_expiration_seconds < BTCM_MAX_TIME_UNTIL_EXPIRATION );
   ...
   annotated_signed_transaction sign_transaction(signed_transaction tx, bool broadcast = false)
   {
      ...
      for( const public_key_type& k : keys_to_use )
      {
         ...
         tx.sign( it->second, BTCM_CHAIN_ID );
      }
      ...
      m["list_my_accounts"] = [](variant result, const fc::variants& a ) {
         ...
         out << "-------------------------------------------------------------------------\n";
            out << std::left << std::setw( 17 ) << "TOTAL"
                << std::right << std::setw(20) << fc::variant( total_btcm, 2 ).as_string() <<" "
   ...
   vector <extended_balance> get_assets (string account)
   {
...
{ try {
   ...
   op.memo_key = memo;
   op.json_metadata = json_meta;
   op.delegation = my->_remote_db->get_chain_properties().account_creation_fee * BTCM_CREATE_ACCOUNT_DELEGATION_RATIO
...
annotated_signed_transaction wallet_api::update_streaming_platform( string streaming_platform_name, string url, bool broadcast )
{
   ...
   op.url=url;
   op.fee = asset(BTCM_MIN_STREAMING_PLATFORM_CREATION_FEE, BTCM_SYMBOL);
...
annotated_signed_transaction wallet_api::vote(string voter, string url, int16_t weight, bool broadcast)
{
   ...
   op.voter = voter;
   op.url = url;
   op.weight = weight * BTCM_1_PERCENT;
...
annotated_signed_transaction      wallet_api::send_private_message( string from, string to, string subject, string body, bool broadcast ) {
   ...
   op.id = BTCM_PRIVATE_MESSAGE_COP_ID;
...
vector <extended_balance> wallet_api::get_assets (string account)
{
