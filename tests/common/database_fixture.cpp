...
#include <btcm/custom_tags/custom_tags.hpp>
...
uint32_t BTCM_TESTING_GENESIS_TIMESTAMP = 1431700000;
...
{ try {
   ...
   auto ahplugin = app.register_plugin< btcm::account_history::account_history_plugin >();
   auto ctplugin = app.register_plugin< btcm::custom_tags::custom_tags_plugin >();
   ...
   for( int i = BTCM_NUM_INIT_MINERS; i < BTCM_MAX_MINERS; i++ )
   {
      account_create( BTCM_INIT_MINER_NAME + fc::to_string( i ), init_account_pub_key );
      fund( BTCM_INIT_MINER_NAME + fc::to_string( i ), BTCM_MIN_PRODUCER_REWARD.amount.value );
      witness_create( BTCM_INIT_MINER_NAME + fc::to_string( i ), init_account_priv_key, "foo.bar", init_account_pub_key, BTCM_MIN_PRODUCER_REWARD.amount );
   }
...
static genesis_state_type prepare_genesis() {
   ...
   result.init_supply = 10000 * asset::scaled_precision( BTCM_ASSET_PRECISION );
   {
      ...
      balance.owner = (std::string) address( balance1.get_public_key() );
   ...
   )
{
   try
   {
      transfer( BTCM_INIT_MINER_NAME, account_name, amount );
   ...
   const asset& amount )
{
   try
   {
      ...
      if ( amount.asset_id == BTCM_SYMBOL )
      {
      ...
      else if ( amount.asset_id == XUSD_SYMBOL )
      {
         ...
         db.adjust_supply( -amount );
         db.adjust_supply(db.to_btcm(amount) );
      }
   } FC_CAPTURE_AND_RETHROW( (account_name)(amount) )
}
...
void database_fixture::vest( const string& from, const share_type& amount )
{
   try
   {
      ...
      op.from = from;
      op.to = "";
      op.amount = asset( amount, BTCM_SYMBOL );
...
void database_fixture::set_price_feed( const price& new_price )
{
   try
   {
      for ( int i = 1; i < 8; i++ )
      {
         ...
         op.publisher = BTCM_INIT_MINER_NAME + fc::to_string( i );
