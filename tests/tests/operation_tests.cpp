...
BOOST_AUTO_TEST_CASE( account_create_authorities )
{
   try
   {
      ...
      op.fee = asset( 10, BTCM_SYMBOL );
      op.new_account_name = "bob";
      op.creator = BTCM_INIT_MINER_NAME;
      ...
      BTCM_REQUIRE_THROW( db.push_transaction( tx, 0 ), tx_missing_active_auth );
      ...
      tx.sign( init_account_priv_key, db.get_chain_id() );
      tx.sign( init_account_priv_key, db.get_chain_id() );
      BTCM_REQUIRE_THROW( db.push_transaction( tx, 0 ), tx_duplicate_sig );

/*      BOOST_TEST_MESSAGE( "--- Test failure when signed by an additional signature not in the creator's authority" );
      tx.signatures.clear();
      tx.sign( init_account_priv_key, db.get_chain_id() );
      tx.sign( alice_private_key, db.get_chain_id() );
      BTCM_REQUIRE_THROW( db.push_transaction( tx, 0 ), tx_irrelevant_sig );
*/
...
BOOST_AUTO_TEST_CASE( account_create_apply )
{
   try
   {
      ...
      const account_object& init = db.get_account( BTCM_INIT_MINER_NAME );
      ...
      op.fee = asset( 100000, BTCM_SYMBOL );
      ...
      auto vests = gpo.total_vesting_fund_btcm;
      ...
      /// because init_witness has created vesting shares and blocks have been produced, 100 BTCM is worth less than 100 vesting shares due to rounding
      ...
      BTCM_REQUIRE_THROW( db.push_transaction( tx, database::skip_transaction_dupe_check ), fc::assert_exception );
      ...
      tx.signatures.clear();
      tx.operations.clear();
      op.fee = asset( db.get_account( BTCM_INIT_MINER_NAME ).balance.amount + 1, BTCM_SYMBOL );
      op.new_account_name = "bob";
      tx.operations.push_back( op );
      tx.sign( init_account_priv_key, db.get_chain_id() );
...
{ try {
   ...
   // invalid delegation
   op.delegation = asset( 100, BTCM_SYMBOL );
   BOOST_CHECK_THROW( op.validate(), fc::assert_exception );
   op.delegation = asset( -100, VESTS_SYMBOL );
   BOOST_CHECK_THROW( op.validate(), fc::assert_exception );
   op.delegation = asset( 100, VESTS_SYMBOL );
...
{ try {
   ...
   db.modify( db.get_witness_schedule_object(), []( witness_schedule_object& w ) {
      w.median_props.account_creation_fee = asset( 300, BTCM_SYMBOL );
      // actually required VESTS for new account:
      // account_creation_fee * BTCM_CREATE_ACCOUNT_DELEGATION_RATIO * vesting_share_price
      // where fee in BTCM counts as fee * BTCM_CREATE_ACCOUNT_DELEGATION_RATIO * vesting_share_price
   ...
   // Alice doesn't have enough BTCM
   op.delegation = asset( 100, VESTS_SYMBOL );
   op.fee = asset( 1000, BTCM_SYMBOL );
   tx.operations.push_back( op );
   tx.sign( alice_private_key, db.get_chain_id() );
   BTCM_REQUIRE_THROW( db.push_transaction( tx, 0 ), fc::assert_exception );
   tx.clear();
   ...
   // Required: 300*5*1 = 1500
   op.fee = asset( 100, BTCM_SYMBOL );
   op.delegation = asset( 1000, VESTS_SYMBOL );
   // Present: 100 BTCM*5*1 + 1000 VESTS
   tx.operations.push_back( op );
   tx.sign( alice_private_key, db.get_chain_id() );
   db.push_transaction( tx, 0 );
   tx.clear();
   ...
   BOOST_CHECK_EQUAL( 1000, bob.received_vesting_shares.amount.value );
   BOOST_CHECK_EQUAL( 100, bob.vesting_shares.amount.value );
   BOOST_CHECK_EQUAL( 1000, alice_new.delegated_vesting_shares.amount.value );
...
*/
      ...
      tx.operations.push_back( op );
      tx.sign( active_key, db.get_chain_id() );
      BTCM_REQUIRE_THROW( db.push_transaction( tx, 0 ), tx_missing_owner_auth );
...
{ try {
   ...
   // invalid amount
   op.vesting_shares = asset( 1, BTCM_SYMBOL );
   BOOST_CHECK_THROW( op.validate(), fc::assert_exception );
   op.vesting_shares = asset( -1, VESTS_SYMBOL );
   BOOST_CHECK_THROW( op.validate(), fc::assert_exception );
   op.vesting_shares = asset( 1, VESTS_SYMBOL );
...
{ try {
   ...
   db.modify( db.get_witness_schedule_object(), []( witness_schedule_object& w ) {
      w.median_props.account_creation_fee = asset( 1, BTCM_SYMBOL );
   });

   generate_block();

   delegate_vesting_shares_operation op;
   op.vesting_shares = asset( 1000, VESTS_SYMBOL );
   op.delegator = "alice";
   op.delegatee = "bob";
   ...
   BOOST_REQUIRE( alice_acc.delegated_vesting_shares == asset( 1000, VESTS_SYMBOL ) );
   BOOST_REQUIRE( bob_acc.received_vesting_shares == asset( 1000, VESTS_SYMBOL ) );
   ...
   BOOST_REQUIRE( delegation->vesting_shares == asset( 1000, VESTS_SYMBOL ) );

   validate_database();

   op.vesting_shares = asset( 2000, VESTS_SYMBOL );
   tx.operations.push_back( op );
   tx.sign( alice_private_key, db.get_chain_id() );
   db.push_transaction( tx, 0 );
   tx.clear();
   ...
   BOOST_REQUIRE( delegation != vd_idx.end() );
   BOOST_REQUIRE( delegation->delegator == op.delegator );
   BOOST_REQUIRE( delegation->vesting_shares == asset( 2000, VESTS_SYMBOL ) );
   BOOST_REQUIRE( alice_acc.delegated_vesting_shares == asset( 2000, VESTS_SYMBOL ) );
   BOOST_REQUIRE( bob_acc.received_vesting_shares == asset( 2000, VESTS_SYMBOL ) );
   ...
   BTCM_REQUIRE_THROW( db.push_transaction( tx ), fc::assert_exception );
   tx.clear();
   ...
   BOOST_REQUIRE( gpo.delegation_return_period == BTCM_DELEGATION_RETURN_PERIOD );
   ...
   generate_blocks( exp_obj->expiration + BTCM_BLOCK_INTERVAL );
...
BOOST_AUTO_TEST_CASE( vote_apply )
{
   try
   {
      ...
      generate_blocks( 60 / BTCM_BLOCK_INTERVAL );
...

      {
         ...
         BOOST_REQUIRE_EQUAL( alice_comment.net_rshares.value, alice.vesting_shares.amount.value * ( old_voting_power - alice.voting_power ) / BTCM_100_PERCENT );
         BOOST_REQUIRE( alice_comment.cashout_time == db.head_block_time() + fc::seconds( BTCM_CASHOUT_WINDOW_SECONDS ) );
         BOOST_REQUIRE( itr->rshares == alice.vesting_shares.amount.value * ( old_voting_power - alice.voting_power ) / BTCM_100_PERCENT );
         ...
         BOOST_REQUIRE_EQUAL( alice.voting_power, old_voting_power - ( old_voting_power * BTCM_100_PERCENT / ( 400 * BTCM_100_PERCENT ) + 1 ) );
         BOOST_REQUIRE_EQUAL( bob_comment.net_rshares.value, alice.vesting_shares.amount.value * ( old_voting_power - alice.voting_power ) / BTCM_100_PERCENT );
         BOOST_REQUIRE( bob_comment.cashout_time == db.head_block_time() + fc::seconds( BTCM_CASHOUT_WINDOW_SECONDS ) );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();
         ...
         generate_blocks( db.head_block_time() + fc::seconds( ( BTCM_CASHOUT_WINDOW_SECONDS / 2 ) ), true );
         ...
         uint128_t new_cashout_time = db.head_block_time().sec_since_epoch() + BTCM_CASHOUT_WINDOW_SECONDS;
         const auto& bob_vote_abs_rshares = ( ( uint128_t( new_bob.vesting_shares.amount.value ) * ( ( BTCM_100_PERCENT / 200 ) + 1 ) ) / ( BTCM_100_PERCENT ) ).to_uint64();

         BOOST_REQUIRE_EQUAL( new_bob.voting_power, BTCM_100_PERCENT - ( BTCM_100_PERCENT / 200 + 1 ) );
         BOOST_REQUIRE_EQUAL( new_alice_comment.net_rshares.value, old_abs_rshares + new_bob.vesting_shares.amount.value * ( old_voting_power - new_bob.voting_power ) / BTCM_100_PERCENT );
         ...
         op.weight = -1 * BTCM_100_PERCENT / 2;
         op.voter = "sam";
         op.author = "bob";
         op.permlink = "foo";
         tx.operations.clear();
         ...
         new_cashout_time = db.head_block_time().sec_since_epoch() + BTCM_CASHOUT_WINDOW_SECONDS;
         auto sam_weight //= ( ( uint128_t( new_sam.vesting_shares.amount.value ) ) / 400 + 1 ).to_uint64();
                         = ( ( uint128_t( new_sam.vesting_shares.amount.value ) * ( BTCM_100_PERCENT / 400 + 1 ) ) / BTCM_100_PERCENT ).to_uint64();

         BOOST_REQUIRE_EQUAL( new_sam.voting_power, BTCM_100_PERCENT - ( BTCM_100_PERCENT / 400 + 1 ) );
         ...
         auto new_rshares = ( ( fc::uint128_t( db.get_account( "alice" ).vesting_shares.amount.value ) * used_power ) / BTCM_100_PERCENT ).to_uint64();
         ...
         old_cashout_time = new_bob_comment.cashout_time.sec_since_epoch();
         new_cashout_time = db.head_block_time().sec_since_epoch() + BTCM_CASHOUT_WINDOW_SECONDS;
         used_power = ( ( ( BTCM_1_PERCENT * 25 * new_alice.voting_power ) / BTCM_100_PERCENT ) / 200 ) + 1;
         auto alice_voting_power = new_alice.voting_power - used_power;

         op.voter = "alice";
         op.weight = BTCM_1_PERCENT * 25;
         op.author = "bob";
         op.permlink = "foo";
         tx.operations.clear();
         tx.signatures.clear();
         ...
         new_rshares = ( ( fc::uint128_t( new_alice.vesting_shares.amount.value ) * used_power ) / BTCM_100_PERCENT ).to_uint64();
         ...
         old_abs_rshares = new_bob_comment.abs_rshares.value;
         old_cashout_time = new_bob_comment.cashout_time.sec_since_epoch();
         used_power = ( uint64_t( BTCM_1_PERCENT ) * 75 * uint64_t( alice_voting_power ) ) / BTCM_100_PERCENT;
         used_power = ( used_power / 200 ) + 1;
         alice_voting_power -= used_power;

         op.weight = BTCM_1_PERCENT * -75;
         tx.operations.clear();
         tx.signatures.clear();
         tx.operations.push_back( op );
         tx.sign( alice_private_key, db.get_chain_id() );
         ...
         generate_blocks( fc::time_point_sec( ( new_bob_comment.cashout_time - BTCM_UPVOTE_LOCKOUT ).sec_since_epoch() + BTCM_BLOCK_INTERVAL ), true );
...
BOOST_AUTO_TEST_CASE( transfer_to_vesting_apply )
{
   try
   {
      ...
      auto vests = asset( gpo.total_vesting_fund_btcm.amount, BTCM_SYMBOL );
      ...
      BOOST_REQUIRE_EQUAL( gpo.total_vesting_fund_btcm.amount.value, vests.amount.value );
      BOOST_REQUIRE_EQUAL( gpo.total_vesting_shares.amount.value, shares.amount.value );
      validate_database();

      op.to = "bob";
      op.amount = asset( 2000000, BTCM_SYMBOL );
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      tx.set_expiration( db.head_block_time() + BTCM_MAX_TIME_UNTIL_EXPIRATION );
...
BOOST_AUTO_TEST_CASE( withdraw_vesting_authorities )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- Test failure when no signature." );
      BTCM_REQUIRE_THROW( db.push_transaction( tx, database::skip_transaction_dupe_check ), tx_missing_active_auth );
      ...
      tx.sign( alice_private_key, db.get_chain_id() );
      BTCM_REQUIRE_THROW( db.push_transaction( tx, database::skip_transaction_dupe_check ), tx_duplicate_sig );

/*      BOOST_TEST_MESSAGE( "--- Test failure with additional incorrect signature" );
      tx.signatures.clear();
      tx.sign( alice_private_key, db.get_chain_id() );
      tx.sign( bob_private_key, db.get_chain_id() );
      BTCM_REQUIRE_THROW( db.push_transaction( tx, database::skip_transaction_dupe_check ), tx_irrelevant_sig );
*/
...
BOOST_AUTO_TEST_CASE( withdraw_vesting_apply )
{
   try
   {
      ...
      BOOST_REQUIRE_EQUAL( db.get_account("alice").vesting_withdraw_rate.amount.value, ( old_vesting_shares.amount / 2 / BTCM_VESTING_WITHDRAW_INTERVALS ).value );
      BOOST_REQUIRE_EQUAL( db.get_account("alice").to_withdraw.value, op.vesting_shares.amount.value );
      BOOST_REQUIRE( db.get_account("alice").next_vesting_withdrawal == db.head_block_time() + BTCM_VESTING_WITHDRAW_INTERVAL_SECONDS );
      validate_database();
      ...
      BOOST_REQUIRE_EQUAL( db.get_account("alice").vesting_shares.amount.value, old_vesting_shares.amount.value );
      BOOST_REQUIRE_EQUAL( db.get_account("alice").vesting_withdraw_rate.amount.value, ( old_vesting_shares.amount / 3 / BTCM_VESTING_WITHDRAW_INTERVALS ).value );
      BOOST_REQUIRE_EQUAL( db.get_account("alice").to_withdraw.value, op.vesting_shares.amount.value );
      BOOST_REQUIRE( db.get_account("alice").next_vesting_withdrawal == db.head_block_time() + BTCM_VESTING_WITHDRAW_INTERVAL_SECONDS );
      validate_database();
...
BOOST_AUTO_TEST_CASE( witness_update_apply )
{
   try
   {
      ...
      op.fee = ASSET( "1.000 2.28.0" );
      op.block_signing_key = signing_key.get_public_key();
      op.props.account_creation_fee = asset( BTCM_MIN_ACCOUNT_CREATION_FEE + 10, BTCM_SYMBOL);
      op.props.maximum_block_size = BTCM_MIN_BLOCK_SIZE_LIMIT + 100;
...
BOOST_AUTO_TEST_CASE( account_witness_vote_authorities )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- Test failure when no signatures" );
      BTCM_REQUIRE_THROW( db.push_transaction( tx, 0 ), tx_missing_basic_auth );
      ...
      tx.signatures.clear();
      tx.sign( sam_private_key, db.get_chain_id() );
      BTCM_REQUIRE_THROW( db.push_transaction( tx, database::skip_transaction_dupe_check ), tx_missing_basic_auth );
...
BOOST_AUTO_TEST_CASE( account_witness_proxy_apply )
{
   try
   {
      ...
      BOOST_REQUIRE_EQUAL( alice.proxy, BTCM_PROXY_TO_SELF_ACCOUNT );
      ...
      BOOST_REQUIRE_EQUAL( sam.proxy, BTCM_PROXY_TO_SELF_ACCOUNT );
      ...
      BOOST_REQUIRE( sam.proxied_vsf_votes_total() == bob.vesting_shares.amount );
      BOOST_REQUIRE_EQUAL( dave.proxy, BTCM_PROXY_TO_SELF_ACCOUNT );
      ...
      tx.operations.clear();
      tx.signatures.clear();
      op.proxy = BTCM_PROXY_TO_SELF_ACCOUNT;
      op.account = "bob";
      tx.operations.push_back( op );
      tx.sign( bob_private_key, db.get_chain_id() );
      ...
      BOOST_REQUIRE_EQUAL( alice.proxy, "sam" );
      BOOST_REQUIRE_EQUAL( alice.proxied_vsf_votes_total().value, 0 );
      BOOST_REQUIRE_EQUAL( bob.proxy, BTCM_PROXY_TO_SELF_ACCOUNT );
      BOOST_REQUIRE_EQUAL( bob.proxied_vsf_votes_total().value, 0 );
      BOOST_REQUIRE_EQUAL( sam.proxy, "dave" );
      ...
      vote.account= "bob";
      vote.witness = BTCM_INIT_MINER_NAME;
      tx.operations.clear();
      tx.signatures.clear();
      ...
      BOOST_REQUIRE( db.get_witness( BTCM_INIT_MINER_NAME ).votes == ( alice.vesting_shares + bob.vesting_shares ).amount );
      validate_database();
      ...
      BOOST_REQUIRE( db.get_witness( BTCM_INIT_MINER_NAME ).votes == bob.vesting_shares.amount );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( feed_publish_apply )
{
   try
   {
      ...
      feed_publish_operation op;
      op.publisher = "alice";
      op.exchange_rate = price( ASSET( "1000.000 2.28.0" ), ASSET( "1.000 2.28.2" ) ); // 1000 BTCM : 1 SBD
...
BOOST_AUTO_TEST_CASE( convert_apply )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- Test failure when account does not have the required BTCM" );
      op.owner = "bob";
      op.amount = ASSET( "5.000 2.28.0" );
      tx.operations.push_back( op );
      tx.sign( bob_private_key, db.get_chain_id() );
      ...
      BOOST_TEST_MESSAGE( "--- Test failure when account does not have the required XUSD" );
      op.owner = "alice";
      ...
      BOOST_TEST_MESSAGE( "--- Test success converting XUSD to BTCM" );
      op.owner = "bob";
...
BOOST_AUTO_TEST_CASE( account_recovery )
{
   try
   {
      ...
      generate_blocks( db.head_block_time() + BTCM_OWNER_UPDATE_LIMIT + fc::seconds(BTCM_BLOCK_INTERVAL) );
      tx.set_expiration( db.head_block_time() + BTCM_MAX_TIME_UNTIL_EXPIRATION );
      ...
      BOOST_REQUIRE( req_itr->expires == db.head_block_time() + BTCM_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD );
      auto expires = req_itr->expires;
      ++req_itr;
      BOOST_REQUIRE( req_itr == request_idx.end() );

      generate_blocks( time_point_sec( expires - BTCM_BLOCK_INTERVAL ), true );
      }
      ...
      generate_blocks( db.head_block_time() + ( BTCM_OWNER_AUTH_RECOVERY_PERIOD - BTCM_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD ) );
      generate_block();
...
BOOST_AUTO_TEST_CASE( change_recovery_account )
{
   try
   {
      ...
      auto recover_account = [&]( const std::string& account_to_recover, const fc::ecc::private_key& new_owner_key, const fc::ecc::private_key& recent_owner_key )
      {
         ...
         // only Alice -> throw
         BTCM_REQUIRE_THROW( db.push_transaction( tx, 0 ), fc::exception );
         tx.signatures.clear();
      ...
      BTCM_REQUIRE_THROW( change_recovery_account("alice", "nobody"), fc::assert_exception );
      BTCM_REQUIRE_THROW( change_recovery_account("haxer", "sam"   ), fc::assert_exception );
      BTCM_REQUIRE_THROW( change_recovery_account("haxer", "nobody"), fc::assert_exception );
      ...
      generate_blocks( db.head_block_time() + BTCM_OWNER_AUTH_RECOVERY_PERIOD - fc::seconds( BTCM_BLOCK_INTERVAL ), true );
      // cannot request account recovery until recovery account is approved
      BTCM_REQUIRE_THROW( request_account_recovery( "sam", sam_private_key, "alice", alice_pub1 ), fc::exception );
      generate_blocks(1);
      // cannot finish account recovery until requested
      BTCM_REQUIRE_THROW( recover_account( "alice", alice_priv1, alice_private_key ), fc::exception );
      // do the request
