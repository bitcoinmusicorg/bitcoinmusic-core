...
/*
      ...
      generate_blocks( db.head_block_time() + BTCM_CONVERSION_DELAY - fc::seconds( BTCM_BLOCK_INTERVAL / 2 ), true );
...
BOOST_AUTO_TEST_CASE( btcm_inflation )
{
   try
   {
   /*
      BOOST_TEST_MESSAGE( "Testing BTCM Inflation until the vesting start block" );
      ...
      auto new_rewards = std::max( BTCM_MIN_CONTENT_REWARD, asset( ( BTCM_CONTENT_APR * gpo.virtual_supply.amount ) / ( BTCM_BLOCKS_PER_YEAR * 100 ), BTCM_SYMBOL ) )
         + std::max( BTCM_MIN_CURATE_REWARD, asset( ( BTCM_CURATE_APR * gpo.virtual_supply.amount ) / ( BTCM_BLOCKS_PER_YEAR * 100 ), BTCM_SYMBOL ) );
      auto witness_pay = std::max( BTCM_MIN_PRODUCER_REWARD, asset( ( BTCM_PRODUCER_APR * gpo.virtual_supply.amount ) / ( BTCM_BLOCKS_PER_YEAR * 100 ), BTCM_SYMBOL ) );
      auto witness_pay_shares = asset( 0, VESTS_SYMBOL );
      auto new_vesting_btcm = asset( 0, BTCM_SYMBOL );
      auto new_vesting_shares = gpo.total_vesting_shares;

      if ( db.get_account( witness_name ).vesting_shares.amount.value == 0 )
      {
         new_vesting_btcm += witness_pay;
         new_vesting_shares += witness_pay * ( gpo.total_vesting_shares / gpo.total_vesting_fund_btcm );
      }

      auto new_supply = gpo.current_supply + new_rewards + witness_pay + new_vesting_btcm;
      new_rewards += gpo.total_reward_fund_btcm;
      new_vesting_btcm += gpo.total_vesting_fund_btcm;
      ...
      BOOST_REQUIRE_EQUAL( gpo.total_reward_fund_btcm.amount.value, new_rewards.amount.value );
      BOOST_REQUIRE_EQUAL( gpo.total_vesting_fund_btcm.amount.value, new_vesting_btcm.amount.value );
      ...
      while( db.head_block_num() < BTCM_START_VESTING_BLOCK - 1)
      {
         ...
         new_rewards = std::max( BTCM_MIN_CONTENT_REWARD, asset( ( BTCM_CONTENT_APR * gpo.virtual_supply.amount ) / ( BTCM_BLOCKS_PER_YEAR * 100 ), BTCM_SYMBOL ) )
            + std::max( BTCM_MIN_CURATE_REWARD, asset( ( BTCM_CURATE_APR * gpo.virtual_supply.amount ) / ( BTCM_BLOCKS_PER_YEAR * 100 ), BTCM_SYMBOL ) );
         witness_pay = std::max( BTCM_MIN_PRODUCER_REWARD, asset( ( BTCM_PRODUCER_APR * gpo.virtual_supply.amount ) / ( BTCM_BLOCKS_PER_YEAR * 100 ), BTCM_SYMBOL ) );
         new_vesting_btcm = asset( 0, BTCM_SYMBOL );
         ...
         if ( db.get_account( witness_name ).vesting_shares.amount.value == 0 )
         {
            ...
            witness_pay = asset( 0, BTCM_SYMBOL );
         }

         new_supply = gpo.current_supply + new_rewards + witness_pay + new_vesting_btcm;
         new_rewards += gpo.total_reward_fund_btcm;
         new_vesting_btcm += gpo.total_vesting_fund_btcm;
      ...
      while( db.head_block_num() < BTCM_START_MINER_VOTING_BLOCK - 1 )
      {
         virtual_supply = gpo.virtual_supply;
         witness_name = db.get_scheduled_witness(1);
         old_witness_balance = db.get_account( witness_name ).balance;
         ...
         witness_pay = std::max( BTCM_MIN_PRODUCER_REWARD, asset( ( BTCM_PRODUCER_APR * gpo.virtual_supply.amount ) / ( BTCM_BLOCKS_PER_YEAR * 100 ), BTCM_SYMBOL ) );
         auto witness_pay_shares = asset( 0, VESTS_SYMBOL );
         new_vesting_btcm = asset( ( witness_pay + new_rewards ).amount * 9, BTCM_SYMBOL );
         new_vesting_shares = gpo.total_vesting_shares;
      ...
      for( int i = 0; i < BTCM_BLOCKS_PER_DAY; i++ )
      {
         virtual_supply = gpo.virtual_supply;
         witness_name = db.get_scheduled_witness(1);
         old_witness_balance = db.get_account( witness_name ).balance;
         ...
         witness_pay = std::max( BTCM_MIN_PRODUCER_REWARD, asset( ( BTCM_PRODUCER_APR * gpo.virtual_supply.amount ) / ( BTCM_BLOCKS_PER_YEAR * 100 ), BTCM_SYMBOL ) );
         witness_pay_shares = witness_pay * gpo.get_vesting_share_price();
         new_vesting_btcm = asset( ( witness_pay + new_rewards ).amount * 9, BTCM_SYMBOL ) + witness_pay;
         new_vesting_shares = gpo.total_vesting_shares + witness_pay_shares;
         new_supply = gpo.current_supply + new_rewards + new_vesting_btcm;
         new_rewards += gpo.total_reward_fund_btcm;
         new_vesting_btcm += gpo.total_vesting_fund_btcm;
      ...
      vesting_btcm = gpo.total_vesting_fund_btcm;
      reward_btcm = gpo.total_reward_fund_btcm;
      ...
      BOOST_REQUIRE_EQUAL( gpo.total_vesting_fund_btcm.amount.value,
         ( vesting_btcm.amount.value
            + ( ( ( uint128_t( virtual_supply.amount.value ) / 10 ) / BTCM_BLOCKS_PER_YEAR ) * 9 )
            + ( uint128_t( virtual_supply.amount.value ) / 100 / BTCM_BLOCKS_PER_YEAR ) ).to_uint64() );
      BOOST_REQUIRE_EQUAL( gpo.total_reward_fund_btcm.amount.value,
         reward_btcm.amount.value + virtual_supply.amount.value / 10 / BTCM_BLOCKS_PER_YEAR + virtual_supply.amount.value / 10 / BTCM_BLOCKS_PER_DAY );
      BOOST_REQUIRE_EQUAL( db.get_account( witness_name ).vesting_shares.amount.value,
         old_witness_shares.amount.value + ( asset( ( ( virtual_supply.amount.value / BTCM_BLOCKS_PER_YEAR ) * BTCM_1_PERCENT ) / BTCM_100_PERCENT, BTCM_SYMBOL ) * ( vesting_shares / vesting_btcm ) ).amount.value );
      validate_database();
      */
   }
   FC_LOG_AND_RETHROW();
...
/*
      ...
      generate_blocks( db.head_block_time() + fc::seconds( BTCM_SBD_INTEREST_COMPOUND_INTERVAL_SEC ), true );
      ...
      BOOST_REQUIRE_EQUAL( db.get_account( "alice" ).mbd_balance.amount.value, alice_mbd.amount.value - ASSET( "1.000 TBD" ).amount.value + ( ( ( ( uint128_t( alice_mbd.amount.value ) * ( db.head_block_time() - start_time ).to_seconds() ) / BTCM_SECONDS_PER_YEAR ) * gpo.mbd_interest_rate ) / BTCM_100_PERCENT ).to_uint64() );
      ...
      generate_blocks( db.head_block_time() + fc::seconds( BTCM_SBD_INTEREST_COMPOUND_INTERVAL_SEC / 2 ), true );
      ...
      generate_blocks( db.head_block_time() + fc::seconds( ( BTCM_SBD_INTEREST_COMPOUND_INTERVAL_SEC * 7 ) / 3 ), true );
      ...
      BOOST_REQUIRE_EQUAL( db.get_account( "alice" ).mbd_balance.amount.value, alice_mbd.amount.value - ASSET( "1.000 TBD" ).amount.value + ( ( ( ( uint128_t( alice_mbd.amount.value ) * ( db.head_block_time() - start_time ).to_seconds() + alice_coindays ) / BTCM_SECONDS_PER_YEAR ) * gpo.mbd_interest_rate ) / BTCM_100_PERCENT ).to_uint64() );
*/
      validate_database();
   }
   FC_LOG_AND_RETHROW();
